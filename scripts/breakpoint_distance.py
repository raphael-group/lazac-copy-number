...
class ChromosomeCopyNumberProfile:
    ...
    def breakpoints(self) -> ChromosomeBreakpointProfile:
        breakpoint_profile = np.hstack(([[1], [1]], self.profile))
...
class CopyNumberProfile:
    ...
    def breakpoints(self) -> BreakpointProfile:
        return BreakpointProfile([p.breakpoints() for p in self.profiles])
...
def breakpoint_magnitude(profile : np.ndarray) -> int:
    ...
    heapq.heapify(positive_entries)
    ...
    while len(positive_entries) > 1:
        ...
        distance += np.abs(t2)
        if t3 != 0:
            heapq.heappush(positive_entries, t3)
    ...
    while len(negative_entries) > 1:
        ...
        distance += np.abs(t3)
        if t3 != 0:
            heapq.heappush(negative_entries, t3)
